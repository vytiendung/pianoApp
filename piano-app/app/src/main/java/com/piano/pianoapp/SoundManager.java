...
public class SoundManager implements Constant {
	...
	public static final String SOUND_PIANO_PATH = "sound/piano";
	 ...
	 * @param instrumentId
	 */
	public void loadDefaultSound(int target, int instrumentId) {
		...
		try {
			...
			String list[] = am.list(pathInAssets);
			String key;
			...
			if(soundIdList.size()==0 ){
				String path=doCheckAndGen(instrumentId);
				loadSoundFromDir(path, target,instrumentId);
			}else{
